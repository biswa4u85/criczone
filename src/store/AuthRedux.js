...
export const getHeadlineList = createAsyncThunk(
  ...
  async (params, { rejectWithValue }) => {
    let urls = `doctype=News+Headlines&limit_page_length=None&filters=${JSON.stringify([["News Headlines", "criczone", "like", 1]])}&fields=${JSON.stringify(["name", "description"])}&cmd=frappe.client.get_list`;
    let response = await apiPostCall('/', urls)
    if (response) {
      return response
    }
...
export const getNewsList = createAsyncThunk(
  ...
  async (params, { rejectWithValue }) => {
    let urls = `doctype=Blog+Post&limit_page_length=None&order_by=published_on desc&filters=${JSON.stringify([["Blog Post", "criczone", "like", 1]])}&fields=${JSON.stringify(["name", "title", "blog_category", "category_description", "blog_intro", "meta_image", "modified", "blogger"])}&cmd=frappe.client.get_list`;
    let response = await apiPostCall('/', urls)
    if (response) {
      return response
    }
...
export const getNewsListByCat = createAsyncThunk(
  ...
  async (params, { rejectWithValue }) => {
    let urls = `doctype=Blog+Post&limit_page_length=None&order_by=published_on desc&filters=${JSON.stringify([["Blog Post", "blog_category", "like", params], ["Blog Post", "criczone", "like", 1]])}&fields=${JSON.stringify(["name", "title", "blog_category", "category_description", "blog_intro", "meta_image", "modified", "blogger"])}&cmd=frappe.client.get_list`;
    let response = await apiPostCall('/', urls)
    if (response) {
      return response
    }
